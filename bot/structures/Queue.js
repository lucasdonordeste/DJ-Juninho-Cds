...
module.exports = class Queue {
    ...
    async _playNext() {
        ...
        if(!nextSong) {
            ...
            this.textChannel.send('Não tem mais música,eu vou sair.');
            return;
        }

        this.textChannel.send(
            new MessageEmbed()
                .setTitle("🎶 Isso aqui ta tocando agora: " + nextSong.info.title)
                .addFields([
                    { inline: true, name: "Cantor(a)", value: nextSong.info.author },
                    { inline: true, name: "Tamanho", value: msToHMS(nextSong.info.length) },
        ...
        if(!this.player) {
            ...
            this.player.on('end', data => {
                if(data.reason === "SUBSTITUÍDO" || data.reason === "PARADO") return;
